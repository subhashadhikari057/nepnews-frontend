...
export default function NewsList({ searchQuery = "" }) {
  ...
  useEffect(() => {
    setNews([]); // Clear previous results before fetching new ones
    fetchNews(1); // Reload news when searchQuery changes
  }, [searchQuery]); // Now it re-runs whenever searchQuery updates
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {news.map((article) => (
          <div key={article._id || `${article.title}-${Math.random()}`} className="bg-white shadow-md rounded-lg p-4">
            <img
            ...
            <p className="text-gray-800 mt-1">
              {article.content ? (article.content.length > 300 ? `${article.content.substring(0, 300)}...` : article.content) : "No content available."}
            </p>
      ...
      {hasMore && (
        <div className="text-center mt-6">
          <button onClick={handleLoadMore} className="bg-blue-500 text-white px-6 py-2 rounded-md hover:bg-blue-600">
            Load More
          </button>
        </div>
      )}
